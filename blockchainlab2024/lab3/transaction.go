...
func NewUTXOTransaction(from, to []byte, amount int, UTXOSet *UTXOSet) *Transaction {
	...
	hash := HashPublicKey(wallet.PublicKey)
	sum, unspentoutputs := UTXOSet.FindUnspentOutputs(hash, amount)
	if sum < amount {
		log.Panic("ERROR")
	}
	var vin []TXInput
	var vout []TXOutput
	for ID, ins := range unspentoutputs {
		txID, _ := hex.DecodeString(ID)
		for _, inp := range ins {
			input := TXInput{txID, inp, nil, wallet.PublicKey}
	...
	if sum > amount {
		output = NewTXOutput(sum-amount, from)
		vout = append(vout, *output)
	}
	trans := Transaction{
		nil,
	...
	trans.SetID()
	UTXOSet.Blockchain.SignTransaction(&trans, wallet.PrivateKey)
	return &trans
}
